...
export class WarehouseLocationComponent implements OnInit {
    ...
    makeList = () => {
        this.paramsToTableData = {
            toolbar: {
                ...
                search: {
                  icon: 'search',
                  propertiesToSearch: ['cd_local', 'descricao']
                }
            },
            list: {
                route: 'LocaisAlmoxarifado',
                crudParams: {
                    order: [{
                        field: 'cd_local',
                        order: 'asc'
    ...
    tableDataOutputReceiver = (e) => {
        if (e.trigger === 'add') {
            const dialogRef = this._dialog.open(WarehouseLocationDialogComponent, {
                width: '95%'
            });
